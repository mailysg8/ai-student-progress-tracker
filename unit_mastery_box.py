...
IMPROVEMENT_FILL    = "#FF9B85"
IMPROVEMENT_STROKE  = "#EE6055"
...
def status_color(status: str, key: str) -> str:
    mapping = {
        ...
        "Needs Attention": {"fill": IMPROVEMENT_FILL, "stroke": IMPROVEMENT_STROKE},
    }
...
def make_legend() -> alt.Chart:
    items = [
        ...
        {"status": "Needs Attention", "fill": IMPROVEMENT_FILL, "stroke": IMPROVEMENT_STROKE, "x": 2},
    ]
...
def unit_mastery(
    ...
    tile_gap: int = 6,
    label_width: int = 90,
    title: str = "Unit Mastery Overview",
) -> alt.VConcatChart:
    """
    ...
    subtitle = (
        f"{n_mastered} KCs mastered  ·  "
        f"{n_progressing} KCs in progress  ·  "
        f"{n_needs} KCs needing attention  ·  "
        f"{n_total} KCs total"
    )
