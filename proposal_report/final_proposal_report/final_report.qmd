...
student_summary = pd.read_csv('/tables/summary_table.csv', index_col='observation')
...
Markdown(practice_summary.to_markdown(index=False))
```
...
1.	**It works on small cohorts.** Deep-learning approaches to knowledge tracing such as Deep Knowledge Tracing (DKT) typically need thousands of observations to train reliably and would overfit on our data. BKT estimates only four parameters per knowledge component, so it produces meaningful estimates from a class of `{python} int(nb_students)` students across `{python} int(nb_kc)` KCs [@badrinath2021].
...
Markdown(kc_agg_results.to_markdown(index=False))
```
...
Together, **RMSE asks whether the predicted probabilities are numerically accurate, while AUC asks whether the model ranks students correctly, placing those who are actually struggling below those who are doing well.** because a model can do well on one and poorly on the other. [@dhanani2014comparison]
...
The Student Overview focuses on one learner. A status summary (@fig-student1) shows the share of
...
A student can access their next steps based on their current mastery levels (@fig-student2). The prerequisite map shows the student's current mastery for each KC, with arrows indicating which skills are required for others. 
