...
- **Sparse coverage.** We discovered that not every student attempted every KC (as can be seen in @tbl-practice_summary). The dashboard treats absent attempts as "not started" rather than as failures.
...
Our goal was to find a way to combine providing insights into how a current class is doing while also guiding data collection for future cohorts. This is a forward-looking problem that required estimates that of a student's mastery update after every new attempt, so that our recommendations always reflect where each student stands now.

**BKT emerged as a good solution**. We selected it as our model for estimation based on four key advantages:
...
2.	**It matches the structure of our data.** The sequential input BKT was designed to model matches what we have.
3.	**It updates without retraining.** Each new response updates a student's mastery estimate, which lets us track students continuously and use the most recent estimate to decide what to assess next.
4.	**Its output is interpretable.** Mastery is expressed as a single probability which can be interpreted as "this student has a 73% chance of having mastered this skill".
...
To assess whether the model generalises to unseen students, we used a student-level 70-30 train-test split. Splitting by student rather than by attempt reflects real deployment conditions and allows us to maintain the sequential structure of the data needed for BKT.

We decided to use two metrics: `RMSE (Root Mean Square Error)` and `AUC (Area Under the Receiver Operating Characteristic Curve)`. Combining a regression metric (`RMSE`) with a classification metric (`AUC`), allows us to test both how accurate the probability estimates are and how well student performance is classified. Both these metrics are also commonly used in BKT research [@dhanani2014comparison].
...
- The Class Overview page (@fig-teacher1) provides information for the whole class. Four summary boxes give insights into how the class is progressing with the KC. The **Unit Mastery Overview** groups every KC into a coloured tile by unit, making weak units visible at a glance. **KC Progress** allows you to compare class progress across selected KCs. **KC Opportunities and Opportunity Heatmap** show how much practice each skill has received and show how students progress across attempts.
...
- The Student Overview page (@fig-teacher2) opens with a student selector, a quantile status toggle, and a status filter , letting users focus on one student and choose between predicted mastery or actual results. Two panels below give complementary views of that student's progress. **Status Summary** shows the percentage of KCs Mastered, Progressing, or Needing Practice, with KC counts beneath each. **P(Mastery)** by KC lists every KC with a progress bar for mastery probability, sorted highest to lowest, surfacing solid skills and weak spots at a glance.
...
- The Data Input page (@fig-teacher3) allows you to **upload CSV files**, while checking that all the column requirements are met. Two buttons then run the uploaded data through **preprocessing and update the dashboard**.

::: {.callout-note}
 **Note** : The Data Input page only works when running the app locally, not on the hosted version.
::: 
