...
The dataset comes from an AP Statistics course, with data collected over 7 months. It contains information about the course structure (how assignments are structured), the knowledge structure (how KC relationships are structured) and the student's performance (scores) for `{python} int(nb_students)` students. The dataset spans `{python} int(nb_assignments)` assignments (includes homeworks and exams for a total of `{python} int(nb_observations)` items) and  `{python} int(nb_kc)` knowledge components, for a total of `{python} format(int(nb_items), ",")` item-level student records.
